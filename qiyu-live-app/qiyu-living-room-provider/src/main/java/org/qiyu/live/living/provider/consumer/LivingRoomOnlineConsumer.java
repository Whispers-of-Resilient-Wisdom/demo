...
/**
 * 直播间用户上线消费者。
 * 该类通过 RocketMQ 消费用户上线消息，并调用 `ILivingRoomService` 进行用户上线处理。
 * 消费者从消息队列中订阅 IM 上线事件，处理用户上线逻辑。
 */
@Component
public class LivingRoomOnlineConsumer implements InitializingBean {
    ...
    /**
     * 初始化时启动 RocketMQ 消费者。
     * 设置消费者相关配置，订阅消息主题，并设置消息监听器来处理上线消息。
     */
    @Override
    public void afterPropertiesSet() throws Exception {
        ...
        mqPushConsumer.setConsumerGroup(rocketMQConsumerProperties.getGroupName() + "_" + LivingRoomOnlineConsumer.class.getSimpleName()); // 设置消费者组
        ...
        mqPushConsumer.subscribe(ImCoreServerProviderTopicNames.IM_ONLINE_TOPIC, ""); // 订阅上线消息主题
